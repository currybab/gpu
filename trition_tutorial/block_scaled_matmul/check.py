...
def check_block_scaled_matmul():
    torch.manual_seed(0)
    for M, N, K, vec_size in ((128, 96, 144, 16), (67, 75, 160, 32)):
        a = torch.randn((M, K), device="cuda", dtype=torch.float16) * 0.25
        b = torch.randn((K, N), device="cuda", dtype=torch.float16) * 0.25
        scale_k = K // vec_size
        a_scale = torch.rand((M, scale_k), device="cuda") + 0.5
        b_scale = torch.rand((scale_k, N), device="cuda") + 0.5
        ...
        print(
            ...
            f"VEC_SIZE={vec_size}, max_abs_error={error:.6f}"
        )
